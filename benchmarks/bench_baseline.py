...
# dependencies = []
# ///
"""Benchmark: stdlib baseline — multiprocessing.Pool (no progress bar).
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("stdlib — multiprocessing.Pool.map (no progress bar)")
    print("=" * 60)
    t0 = time.perf_counter()
    with Pool(8) as pool:
        results = pool.map(price_option, seeds)
