...
# dependencies = ["mpire[dashboard]"]
# ///
"""Benchmark: MPIRE — MultiProcessing Is Really Easy.
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("MPIRE — WorkerPool.map with progress bar")
    print("=" * 60)
    t0 = time.perf_counter()
    with WorkerPool(n_jobs=8) as pool:
        results = pool.map(price_option, seeds, progress_bar=True)
    elapsed = time.perf_counter() - t0
    avg = sum(r["price"] for r in results) / len(results)
    print(f"{len(results)} options priced in {elapsed:.1f}s  (avg price: ${avg:.4f})\n")

    # --- Run 2: with worker insights ---
    print("=" * 60)
    print("MPIRE — with worker insights enabled")
    print("=" * 60)
    t0 = time.perf_counter()
    with WorkerPool(n_jobs=8) as pool:
        results = pool.map(price_option, seeds, progress_bar=True, enable_insights=True)
        insights = pool.get_insights()
    elapsed = time.perf_counter() - t0
    avg = sum(r["price"] for r in results) / len(results)
    print(f"{len(results)} options priced in {elapsed:.1f}s  (avg price: ${avg:.4f})")
    print(f"Worker insights: {insights}")
