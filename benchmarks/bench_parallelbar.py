...
# dependencies = ["parallelbar"]
# ///
"""Benchmark: parallelbar — parallel map with tqdm + error handling.
...
if __name__ == "__main__":
    ...
    # --- Run 1: basic progress bar ---
    print("=" * 60)
    print("parallelbar — progress_map with tqdm")
    print("=" * 60)
    t0 = time.perf_counter()
    results = progress_map(price_option, seeds, n_cpu=8)
    elapsed = time.perf_counter() - t0
    avg = sum(r["price"] for r in results) / len(results)
    print(f"{len(results)} options priced in {elapsed:.1f}s  (avg price: ${avg:.4f})\n")

    # --- Run 2: error coercion (like pmap's safe_mode) ---
    print("=" * 60)
    print("parallelbar — error_behavior='coerce' (safe mode equivalent)")
    print("=" * 60)
    ...
    t0 = time.perf_counter()
    results = progress_map(flaky_price, seeds, n_cpu=8, error_behavior="coerce")
    elapsed = time.perf_counter() - t0
    errors = [r for r in results if r is None]
    print(f"{len(results)} tasks ({len(errors)} errors coerced to None) in {elapsed:.1f}s")
